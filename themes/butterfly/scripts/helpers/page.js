...
/**
 * 移除 HTML 中的 <pre> 和 <code> 标签及其内容
 * @param {string} html - 包含 HTML 标签的字符串
 * @returns {string} - 移除代码块后的字符串
 */
function stripCodeBlocks(html) {
  // 先移除 <pre> 块（包含嵌套的 <code>）
  let result = html.replace(/<pre[^>]*>[\s\S]*?<\/pre>/gi, '')
  // 再移除独立的 <code> 标签（行内代码）
...
hexo.extend.helper.register('page_description', function () {
  ...
  if (description) {
    description = escapeHTML(stripHTML(stripCodeBlocks(description)).substring(0, 150)
      .trim()
    ).replace(/\n/g, ' ')
    return description
  }
